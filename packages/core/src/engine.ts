...
function segmentLength(line: LineSegment): number {
  const dx = line.x2 - line.x1;
  const dy = line.y2 - line.y1;
  return Math.hypot(dx, dy);
}

function lineMidpoint(line: LineSegment): { x: number; y: number } {
...
function estimateConfidence(lines: LineSegment[], previous?: DetectionResult): number {
...
function lineAngle(line: LineSegment): number {
...
function undirectedAngleDelta(a: number, b: number): number {
...
function templateEdgeAngle(edge: EdgeGraphEdge): number {
  return Math.atan2(edge.b[1] - edge.a[1], edge.b[0] - edge.a[0]);
}

function templateEdgeWeight(edge: EdgeGraphEdge): number {
  const weight = edge.weight ?? 1;
  return Number.isFinite(weight) && weight > 0 ? weight : 1;
}

function bestObservedSimilarity(
  observedAngles: number[],
  templateAngle: number,
  yaw: number,
): number {
  if (observedAngles.length === 0) return 0;
  let best = 0;
...
function estimateYawFromTemplate(
  lines: LineSegment[],
  edges: EdgeGraphEdge[],
): { yaw: number; orientationScore: number; weightedCoverage: number; matchedTags: Set<string> } {
...
function requiredTagFactor(requiredTags: string[] | undefined, matchedTags: Set<string>): number {
  if (!requiredTags || requiredTags.length === 0) return 1;
  const normalized = requiredTags.filter((tag) => tag.trim().length > 0);
  if (normalized.length === 0) return 1;
  let matched = 0;
  for (const tag of normalized) {
    if (matchedTags.has(tag)) matched += 1;
  }
...
function makeDetection(
  ...
  const { yaw, orientationScore, weightedCoverage, matchedTags } = estimateYawFromTemplate(
    lines,
    options.shapeModel.edges,
  );
  ...
  const confidenceRaw = Math.max(
    0,
    Math.min(1, (baseConfidence * 0.5) + (orientationScore * 0.35) + (templateCoverage * 0.15)),
  );
...
export function estimatePose(input: PoseEstimateInput): DetectionResult[] {
